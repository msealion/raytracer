...
fn raycast_sphere() {
    ...
    camera
        .render(&world)
        .unwrap()
        .output_to_ppm("resources/test_outputs/test_raycast_sphere.ppm")
        .unwrap();
}

#[test]
#[ignore]
fn raycast_scene_native_resolution() {
    ...
    image
        .output_to_ppm("resources/test_outputs/test_output_raycast_scene_native.ppm")
        .unwrap();
}

#[test]
#[ignore]
fn raycast_scene_agss() {
    let floor = Sphere::builder()
        .set_frame_transformation(Transform::new(TransformKind::Scale(10.0, 0.01, 10.0)))
        .set_material(Material {
            pattern: Box::new(Solid::new(Colour::new(1.0, 0.9, 0.9))),
    ...
    let image = camera.render(&world).unwrap();
    image
        .output_to_ppm("resources/test_outputs/test_output_raycast_scene_agss.ppm")
        .unwrap();
}
