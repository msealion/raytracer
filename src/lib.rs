...
Two types of re-exports are commonly used:
- Crate-level re-exports: re-exports used within the crate; every name is
exposed
- Public re-exports: re-exports in the public interface; only commonly used
names are exposed

## Public re-exports

For public re-exports, all exporting is done through flattening preludes. Each
module, including the top-level module, will import all names in the preludes
of the immediate nested modules using the glob operator. These names should be
commonly used to be included in the prelude.
...
For crate-level re-exports, all names are re-exported to de-nest exactly one
level of nesting. This only applies to modules containing terminal submodules
(that is, submodules that do not have sub-submodules).
